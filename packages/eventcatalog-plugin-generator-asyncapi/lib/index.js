...
const parseAsyncAPIFile = async (pathToFile, options, copyFrontMatter) => {
    ...
    if (domainName) {
        const { writeDomainToCatalog } = (0, utils_1.default)({ catalogDirectory: process.env.PROJECT_DIR });
        const domain = {
            name: domainName,
    ...
    const { writeServiceToCatalog, writeEventToCatalog } = (0, utils_1.default)({
        catalogDirectory: domainName ? path_1.default.join(process.env.PROJECT_DIR, 'domains', domainName) : process.env.PROJECT_DIR,
    });
